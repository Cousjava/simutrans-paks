#!/bin/bash
echo -e "\033[0;95m Erstelle Fabriken"
cd ./Apotheke
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ > "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Autohandel
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Baeckerei
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Baumarkt
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Baustellen
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Beton
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Brauerei
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Brennstoffhandel
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Buecher
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Chemiefabrik
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Computerhersteller
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Discounter
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Elektrohandel
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Getraenkehandel
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd 160
$repo/makeobj PAK160 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ..
cd ../Glas
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../ImportExport
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Kameras
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Kaufhaus
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Kelterei
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Kfz-Fabrik
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Kieswerk
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Kokerei
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Kraftwerke
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Krankenhaus
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Lebensmittelfabrik
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Markt
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Marzipanfabrik
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Maschinen
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Milchwerk
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Moebel
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Muehlen
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Muell
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Papierfabrik
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Pharmawerk
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Raffinerie
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Saegewerk
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Schwimmdock
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Schlachthof
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../SchrottHandel
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Silo
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Stahlwerk
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Supermarkt
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Tankstelle
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Textilfabrik
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Tierfutterfabrik
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Verpackungsmittel
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Versandhandel
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Verwaltung
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Werft
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Zement
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Zuckerfabrik
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../
cd ./Rohstoffe
echo "Erstelle Rohstoff Produzenten"
cd ./Erz
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ > "$repo/LOG/LOG_rohstoffe.log" 2>> "$repo/LOG/error_rohstoffe.log"
cd ../Felder
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/LOG_rohstoffe.log" 2>> "$repo/LOG/error_rohstoffe.log"
cd ../Fische
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/LOG_rohstoffe.log" 2>> "$repo/LOG/error_rohstoffe.log"
cd ../Getreide
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/LOG_rohstoffe.log" 2>> "$repo/LOG/error_rohstoffe.log"
cd ../Holz
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/LOG_rohstoffe.log" 2>> "$repo/LOG/error_rohstoffe.log"
cd ../Hopfen
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/LOG_rohstoffe.log" 2>> "$repo/LOG/error_rohstoffe.log"
cd ../Kalisalz
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/factory.log" 2>> "$repo/LOG/error_factory.log"
cd ../Kohle
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/LOG_rohstoffe.log" 2>> "$repo/LOG/error_rohstoffe.log"
cd ../Obst
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/LOG_rohstoffe.log" 2>> "$repo/LOG/error_rohstoffe.log"
cd ../Oel
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/LOG_rohstoffe.log" 2>> "$repo/LOG/error_rohstoffe.log"
cd ../Sand
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/LOG_rohstoffe.log" 2>> "$repo/LOG/error_rohstoffe.log"
cd ../Steine
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/LOG_rohstoffe.log" 2>> "$repo/LOG/error_rohstoffe.log"
cd ../Vieh
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/LOG_rohstoffe.log" 2>> "$repo/LOG/error_rohstoffe.log"
cd ../Wein
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/LOG_rohstoffe.log" 2>> "$repo/LOG/error_rohstoffe.log"
cd ../Zuckerrueben
$repo/makeobj PAK128 $repo/simutrans/PAK128.german/ ./ >> "$repo/LOG/LOG_rohstoffe.log" 2>> "$repo/LOG/error_rohstoffe.log"
cd $repo
