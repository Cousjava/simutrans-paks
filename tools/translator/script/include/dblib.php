...
//                      echo 'Success... ' . mysqli_get_host_info($st_dbi) . "\n";
   ...
   { $versions_all =array();
     $sql = "SELECT v_name, version_id FROM versions WHERE activ = 1 ORDER BY version_id ASC";
     $vsr = mysqli_query($st_dbi,$sql) or die ("SQL error: ".mysqli_error($st_dbi));
     while ($row=mysqli_fetch_array($vsr)) $versions_all[$row['version_id']]=$row['v_name'];
