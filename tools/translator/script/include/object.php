...
// give warning, if a paramter in the .dat file is double
function double_warning ($obj,$tokens,$old)
{ global $LNG_OBJ_IMPORT;
  echo "<p class='red'><h3>Object: ".$obj."<br>".
  $LNG_OBJ_IMPORT[29]."<br>".
  $tokens[0]."=".$old."<br>".
...
class simu_object
{
    ...
    */
    ...
    function is_valid ()
    {   global $tr_name_pattern,$sub_waytypes,$object_text,$already_imported_obj;
        ...
        { echo "<p class='red'><h3>Error dummy_info Object found. This is outdated way for detail text.</h3></p>\n";
          return false;
        }
        ...
        { if ($tv == '?') continue;
          ...
          { printf("<p class='red'><h3>%s<br>Error reserved Object Name found. It is not allowed to use special text names as object name</h3></p>\n",$this->name);
            return false;
          }
        }

        // test for doppelt in one import
        if (in_array($this->name,$already_imported_obj))
        { printf("<p class='red'><h3>This Object is already imported in this run!<br>Object %s is double in your zip-file</h3></p>\n",$this->name);
          return false;
        }
        $already_imported_obj[] = $this->name;
    ...
    function save_attribute ($tokens)
    {
        ...
        if ($tokens[0] == "obj")
        {   if ($this->obj != "") double_warning ($this->name,$tokens,$this->obj);
           ...
           AND (preg_match("#^freightimagetype#i"  , $tokens[0]) != 1)) 
        {
            ...
            //save the image line
            if (isset($this->images [$tokens[0]]) and 
                      $this->images [$tokens[0]] != "") 
               double_warning ($this->name,$tokens,$this->images [$tokens[0]][0]);
            $this->images [$tokens[0]] = array($tokens[1], $unzoom);
            return;
        }

        //if we got here, then it is normal attribute
        if (isset($this->attributes [$tokens[0]]) and 
                  $this->attributes [$tokens[0]] != "") 
           double_warning ($this->name,$tokens,$this->attributes [$tokens[0]]);
 ...
 //       echo 'kommentar nach p'.$wl[1].'<br>';
        ...
        if (trim($line) == "") return;
        //now we are sure that line is not empty and has not only comment
        //we were modified
        $this->modified = TRUE;
 ...
 //     echo "z+".$line."+z<br>";
        ...
        {   //in normal case we want to trim the line
            ...
            if ($tokens[0] == "name") $this -> untrimmed_name = rtrim ($tokens[1], "\n\r");
    ...
    function save_1image_to_db (&$im_old_id,$img_name, $source_image_path,$zoomable, $image, $tilesize,$pos_x,$pos_y,$offset_x,$offset_y)
	...
	if ($img_size == 0)
          ...
          if (ob_get_length () >= 16777215)
          { ob_end_clean ();//disble buffering and go to next
            echo "<h3>Tried to save image tile: ".basename($source_image_path)." . $pos_y . $pos_x , but its size ($img_size) exceeds 16777215B and will not fit into the database.</h3>";
            return False;
          }
    ...
    {   global $im_unmodified, $im_updated, $im_deleted, $im_inserted;
        ...
        foreach ($this->images as $img_name => $value)
        {
            ...
            $this -> save_1image_to_db ($im_old_id,$img_name,$source_image_path,$zoomable, $tile, $this->tile_size,$pos_x,$pos_y,$offset_x,$offset_y);

            //cleanup
            imagedestroy ($tile);
	  } else {
            $this -> save_1image_to_db ($im_old_id,$img_name,$source_image_path,$zoomable, NULL,  $this->tile_size,0,0,0,0);
	  }
        }
