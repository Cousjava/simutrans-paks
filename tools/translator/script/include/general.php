...
function PrintTitle() {
  global $title;
  echo "<h1 class=\"pagename\">".$title."</h1>\n";
}
...
function pakset_activ()
{
    ...
    { if ( $row['activ'] == 1 )
      {   $enabled .= ' &nbsp;&nbsp;"'.$row['v_name'].'"&nbsp;&nbsp; ';
          $set_enabled[] = $row['version_id'];
      } elseif ( $row['activ'] == 0 ) 
      {   $disabled .= ' &nbsp;&nbsp;"'.$row['v_name'].'"&nbsp;&nbsp; ';
          $set_disabled[] = $row['version_id'];
      }
    
      if ( $row['maintainer_user_id']  == $_SESSION['userId'] || 
           $row['maintainer_user_id2'] == $_SESSION['userId'] || 
           $row['maintainer_user_id3'] == $_SESSION['userId'] ) 
      {  $maintainter[] = $row['version_id'];
      }
    }
    db_free_result($versions);
    
    $_SESSION['maintainter'] = $maintainter;
    $_SESSION['set_enabled'] = $set_enabled;
    $_SESSION['set_disabled'] = $set_disabled;
    
    $set_activ[0] = $enabled;
    $set_activ[1] = $disabled;
    
    return $set_activ;
}    

function get_maintainter() {
  // the user is maintainter from sets
        $version_sql = "SELECT `v_name`,`version_id`,`maintainer_user_id`,`maintainer_user_id2`,`maintainer_user_id3` FROM `versions`";
        ...
        {   
          if ( $row['maintainer_user_id']  == $_SESSION['userId'] || 
               ...
               $row['maintainer_user_id3'] == $_SESSION['userId'] ) 
          {
            $set_admin[$g][0] = $row['version_id'];
            $set_admin[$g][1] = $row['v_name'];
            $g++;
          } 
        }
...
function get_setmaintainter($set) {
  // maintainter from set
  $version_sql = "SELECT * FROM `versions` WHERE `version_id`=".$set.";";
  $db_result = db_query($version_sql);
  ...
  $set_admin[0] = $row['maintainer_user_id'];
  $set_admin[1] = $row['maintainer_user_id2'];
  $set_admin[2] = $row['maintainer_user_id3'];
