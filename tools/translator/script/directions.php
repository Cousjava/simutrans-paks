...
{   GLOBAL   $LNG_EDIT, $LNG_ADMIN;
    ...
    { if ($ot != $object['obj'])   
        ...
        if ( !isset($_SESSION['userId'])) { $user = '*'; } else { $user = $_SESSION['userId']; }
        if ( in_array($user, get_setmaintainter($versin_id)) ) {
...
{ if (!$sr_string2 and $language == 255 and $trans_auswahl == 'untranslated') // too mutch  
  ...
  {  $obj_id = $sr['object_id'];
     ...
     // set admin links
     if ( !isset($_SESSION['userId']) ) { $user = '*'; } else { $user = $_SESSION['userId']; }
     if ( in_array($user, get_setmaintainter($version_auswahl)) ) 
