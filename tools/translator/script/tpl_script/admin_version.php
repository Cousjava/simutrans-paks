...
function ver_info ($ver) {
  ...
  $query = db_query($sql);
  $pos = 0;
  $v_att['form_add_version']['comaintainter1'][$pos]['id_maintainter'] = 'none';
  $v_att['form_add_version']['comaintainter1'][$pos]['name_maintainter'] = 'none';
    
  $v_att['form_add_version']['comaintainter2'][$pos]['id_maintainter'] = 'none';
  $v_att['form_add_version']['comaintainter2'][$pos]['name_maintainter'] = 'none';
  
  while ($usr=db_fetch_array($query)) {  
    // Auswahlliste Maintainter
    $v_att['form_add_version']['maintainter'][$pos]['id_maintainter'] = $usr['u_user_id'];
    $v_att['form_add_version']['maintainter'][$pos]['name_maintainter'] = $usr['real_name'];
    if ( $usr['u_user_id'] == $row['maintainer_user_id'] ) { 
      //$v_att['form_add_version']['maintainter'][$pos]['value_maintainter'] = $usr['real_name'];
      $v_att['form_add_version']['maintainter'][$pos]['select_m'] = 'SELECTED="SELECTED"'; 
    } else {  
      $v_att['maintainter'][$pos]['select_m'] = ' '; 
    }
    
    $pos++;
    
    // Auswahlliste Co-Maintainter 1
    $v_att['form_add_version']['comaintainter1'][$pos]['id_maintainter'] = $usr['u_user_id'];
    $v_att['form_add_version']['comaintainter1'][$pos]['name_maintainter'] = $usr['real_name'];
    if ( $usr['u_user_id'] == $row['maintainer_user_id2'] ) { 
      //$v_att['form_add_version']['comaintainter1'][$pos]['value_maintainter2'] = $usr['real_name'];
      $v_att['form_add_version']['comaintainter1'][$pos]['select_m1'] = 'SELECTED="SELECTED"'; 
    } else {  
      $v_att['form_add_version']['comaintainter1'][$pos]['select_m1'] = ' '; 
    }
    
    // Auswahlliste Co-Maintainter 2
    $v_att['form_add_version']['comaintainter2'][$pos]['id_maintainter'] = $usr['u_user_id'];
    $v_att['form_add_version']['comaintainter2'][$pos]['name_maintainter'] = $usr['real_name'];
    if ( $usr['u_user_id'] == $row['maintainer_user_id3'] ) { 
      //$v_att['form_add_version']['comaintainter2'][$pos]['value_maintainter3'] = $usr['real_name'];
      $v_att['form_add_version']['comaintainter2'][$pos]['select_m2'] = 'SELECTED="SELECTED"'; 
    } else {  
      $v_att['form_add_version']['comaintainter2'][$pos]['select_m2'] = ' '; 
    }
...
function ver_post () { 
  ...
  if ($_POST['mode']=="add") {
    ...
    $sql="INSERT INTO `versions` (`v_name`,`tile_size`,`maintainer_user_id`,`maintainer_user_id2`,`maintainer_user_id3`,`version_id`) ".
      " VALUES ( '%s',%d,'%s','%s','%s',%d );";
    // new Table for Set
  ...
  if ($_POST['mode']=="edit") {
    ...
    $sql="UPDATE `versions` SET `v_name`='%s',`tile_size`=%d,`maintainer_user_id`='%s',`maintainer_user_id2`='%s',`maintainer_user_id3`='%s' ".
      " WHERE `version_id`='%s';";
  ...
  if ( $_POST['ver_maintainer2'] == 'none' ) { $u2 = NULL; } else { $u2 = $_POST['ver_maintainer2']; }
  if ( $_POST['ver_maintainer3'] == 'none' ) { $u3 = NULL; } else { $u3 = $_POST['ver_maintainer3']; }
  
  $sql = sprintf ($sql
    ,$_POST['ver_name']
    ,$_POST['tile_size']
    ,$_POST['ver_maintainer']
    ,$u2
    ,$u3
    ,$ver
  );
