<?PHP
  ...
  $v_att['page_title'] =  $page_titel[$title];
    ...
    while ($row=db_fetch_array($query)) {
      ...
      $cm = '';
      if ( $row['maintainer_user_id2'] != NULL ) $cm .= $row['maintainer_user_id2'];
      if ( $row['maintainer_user_id3'] != NULL ) $cm .= ', '.$row['maintainer_user_id3'];
      if ( $row['maintainer_user_id4'] != NULL ) $cm .= ', '.$row['maintainer_user_id4'];
      if ( $row['maintainer_user_id5'] != NULL ) $cm .= ', '.$row['maintainer_user_id5'];
      if ( $row['maintainer_user_id6'] != NULL ) $cm .= ', '.$row['maintainer_user_id6'];
      if ($cm !='')
      { $v_att['setlist'][$x]['set_comaintainter']['bez_comaintainter'] = $LNG_INFO[2];
        $v_att['setlist'][$x]['set_comaintainter']['set_maintainter2'] = $cm; 
      }
