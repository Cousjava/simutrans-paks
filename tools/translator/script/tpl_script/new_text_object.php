<?PHP
  ...
  /* messages */
    ...
    $i = 0; 
      ...
      for ( $x = 0; $x < count($set_admin); $x++ ) {
            if ( $set_admin[$x][0] == $version_auswahl ) { 
              $t = "selected='selected'"; 
            } else {
              $t = "";
            }
            $v_att_page['object_data'][$i]['select_field']['list_options'][$x]['opt_value'] = $set_admin[$x][0];  
            $v_att_page['object_data'][$i]['select_field']['list_options'][$x]['opt_name'] = $set_admin[$x][1];  
            $v_att_page['object_data'][$i]['select_field']['list_options'][$x]['opt_select'] = $t;  
            $setname[$set_admin[$x][0]] = $set_admin[$x][1];               
      }
