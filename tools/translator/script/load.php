...
/*
  //access check
  $translate = db_fetch_array(db_query("SELECT count(*) FROM translate WHERE lng_tr_language_id='$language' AND translator_user_id='$user'"));
  //$user_role = db_one_field_query ("SELECT role FROM users WHERE u_user_id='$userId';");
  $user_role = $_SESSION['role'];
  if (($translate[0]==0) and ($user_role != "admin"))
  {   printf("<b>$LNG_LOAD3[2]<br /></b>", $user, $language);
      ...
      { //get current location of uploaded file (some temp)
        ...
        { echo "Extracting the ".$file_name." file...<br>\n";
          ...
          { $pfile=$v_entry['filename'];
            ...
            if (substr($file_name,-4) == ".tab" and substr($file_name,0,-4) != $language)
              printf($LNG_LOAD3[0],$file_name);
            else tr_parsetab($file_name,$pfile,$language,$version_auswahl,3,0,NULL);
          }
          verzeichnis_del($tmp);
        } elseif (substr($file_name,-4) == ".tab" and substr($file_name,0,-4) != $language)
           printf($LNG_LOAD3[0],$file_name);
