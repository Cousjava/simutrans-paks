...
// update status set
if ( isset($_GET['set']) && in_array($_GET['set'],  $_SESSION['maintainter']) ) { 
...
if ( isset($_POST['set_lang']) ) {
    ...
    for ( $x = 0; $x < count($_SESSION['maintainter']); $x++ ) {
          if ($_POST['modifid_'.$_SESSION['maintainter'][$x]]) { $setid = $_SESSION['maintainter'][$x]; }
    }
...
// show status set
if ( is_null($_SESSION['maintainter']) ) {
  $_SESSION['maintainter'] = array();
}
...
{
  if ( in_array($row['version_id'],  $_SESSION['maintainter']) ) {
  /*   if ( $row['activ'] == 1 ) {
