...
#  endif
	...
	if(is_display_init()) {
		...
		sel->set_window_size( sel->get_window_size()+koord(150,0) );

		koord xy( display_get_width()/2 - 180, display_get_height()/2 - sel->get_window_size().y/2 );
		event_t ev;
		...
		while(win_is_top(sel)) {
			...
			sel->zeichnen( xy, sel->get_window_size() );
