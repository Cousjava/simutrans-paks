	...
	for(uint8 i=0; i<freight_builder_t::get_max_catg_index(); i++) {
		...
		if (warray) {
			for(uint32 j=0; j<warray->get_count(); j++) {
				halt->add_freight_to_halt( (*warray)[j] );
			}
			delete waren[i];
			waren[i] = NULL;
		}
