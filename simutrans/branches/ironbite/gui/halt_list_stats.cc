...
void halt_list_stats_t::zeichnen(koord offset)
{
	...
	if(  !( (pos.y+offset.y)>clip.yy  ||  (pos.y+offset.y)<clip.y-32 )  &&  halt.is_bound()) {
		...
		// status now in front
		display_ddd_box_clip(pos.x+offset.x+5, pos.y+offset.y+5, 32, D_INDICATOR_HEIGHT+2, MN_GREY0, MN_GREY4);
		display_fillbox_wh_clip(pos.x+offset.x+6, pos.y+offset.y+6, 30, D_INDICATOR_HEIGHT, halt->get_status_farbe(), true);

		// name
		int left = pos.x + offset.x + 28 + display_proportional_clip(pos.x + offset.x + 39 +4, pos.y + offset.y + 3, translator::translate(halt->get_name()), ALIGN_LEFT, COL_BLACK, true);

		// what kind of stop
		haltestelle_t::stationtyp const halttype = halt->get_station_type();
		int pos_y = pos.y+offset.y-42;
		...
		pos_y = pos.y+offset.y+15;
		left = pos.x+offset.x+6;
		...
		{
			display_proportional_clip(left+1, pos_y, "-", ALIGN_LEFT, MN_GREY0, true);
		}
		...
		display_proportional_clip(pos.x+offset.x+39+4, pos_y, buf, ALIGN_LEFT, COL_BLACK, true);
	}
}
