...
/**
 * Default filter: keine �lbohrinseln!
 */
int halt_list_frame_t::filter_flags = 0;

char halt_list_frame_t::name_filter_value[64] = "";

slist_tpl<const freight_desc_t *> halt_list_frame_t::waren_filter_ab;
slist_tpl<const freight_desc_t *> halt_list_frame_t::waren_filter_an;
...
bool halt_list_frame_t::passes_filter(halthandle_t halt)
{
	...
	if(get_filter(ware_ab_filter)) {
		...
		for(i = 0; !ok && i < freight_builder_t::get_waren_anzahl(); i++) {
			const freight_desc_t *ware = freight_builder_t::get_info(i);

			if(get_ware_filter_ab(ware)) {
				if(ware == freight_builder_t::passagiere) {
					ok = halt->get_pax_enabled();
				}
				else if(ware == freight_builder_t::post) {
					ok = halt->get_post_enabled();
				}
				else if(ware != freight_builder_t::nichts) {
	...
	filter_label(translator::translate("hl_txt_filter"))
{
	...
	set_window_size(koord(TOTAL_WIDTH, TITLEBAR_HEIGHT+7*(28)+31+1));
	set_min_window_size(koord(TOTAL_WIDTH, TITLEBAR_HEIGHT+3*(28)+31+1));
...
void halt_list_frame_t::set_ware_filter_ab(const freight_desc_t *ware, int mode)
{
...
void halt_list_frame_t::set_ware_filter_an(const freight_desc_t *ware, int mode)
{
	if(ware != freight_builder_t::nichts) {
		if(get_ware_filter_an(ware)) {
			if(mode != 1) {
...
void halt_list_frame_t::set_alle_ware_filter_ab(int mode)
{
	if(mode == 0) {
		...
		for(unsigned int i = 0; i<freight_builder_t::get_waren_anzahl(); i++) {
			set_ware_filter_ab(freight_builder_t::get_info(i), mode);
		}
	}
}
...
void halt_list_frame_t::set_alle_ware_filter_an(int mode)
{
	if(mode == 0) {
		...
		for(unsigned int i = 0; i<freight_builder_t::get_waren_anzahl(); i++) {
			set_ware_filter_an(freight_builder_t::get_info(i), mode);
		}
	}
}
