	...
	gui_frame_t( translator::translate("Spielerliste") )
{
	...
	for(int i=0; i<MAX_PLAYER_COUNT-1; i++) {
		...
		player_select[i].append_element( new const_text_scrollitem_t( translator::translate("slot empty"), COL_BLACK ) );
		player_select[i].append_element( new const_text_scrollitem_t( translator::translate("Manual (Human)"), COL_BLACK ) );
		if(  !welt->get_spieler(1)->is_locked()  ||  !umgebung_t::networkmode  ) {
			player_select[i].append_element( new const_text_scrollitem_t( translator::translate("Goods AI"), COL_BLACK ) );
			player_select[i].append_element( new const_text_scrollitem_t( translator::translate("Passenger AI"), COL_BLACK ) );
		}
