...
void citybuilding_edit_frame_t::fill_list( bool translate )
{
	...
	if(bt_res.pressed) {
		...
		{
			haus_besch_t const *  besch = s->get(i);
	...
	for(uint32 i=0; i<hauslist.get_count(); i++)
	{
		haus_besch_t const * tmp = hauslist.get(i);
		...
		char const* const name = translate ? translator::translate(tmp->get_name()) : tmp->get_name();
