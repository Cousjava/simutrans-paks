	...
	ware_scrolly(&ware_cont)
{
	...
	for(i=n=0; i<freight_builder_t::get_waren_anzahl(); i++) {
	...
	for(i=1; i<freight_builder_t::get_max_catg_index(); i++) {
		if(freight_builder_t::get_info_catg(i)->get_catg()!=0) {
			ware_item_t *item = new ware_item_t(this, freight_builder_t::get_info_catg(i));
			item->init(button_t::square, translator::translate(freight_builder_t::get_info_catg(i)->get_catg_name()), koord(16, 16*n++ + 4));
			ware_cont.add_komponente(item);
		}
	}
